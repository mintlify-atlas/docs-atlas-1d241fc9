...
<Note>
  This project uses Expo's New Architecture (`newArchEnabled: true`), React Compiler (`reactCompiler: true`), and typed routes (`typedRoutes: true`). All three are enabled in `app.json` under the `experiments` block, providing better runtime performance, automatic memoization, and compile-time route safety respectively.
</Note>
